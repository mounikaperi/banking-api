...
public class SecurityConfig {
    ...
    SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        http.sessionManagement(session -> session.invalidSessionUrl("/invalidSession").maximumSessions(3).maxSessionsPreventsLogin(true))
                .requiresChannel(channel -> channel.anyRequest().requiresInsecure()) // allow only https
                .csrf(csrfConfig -> csrfConfig.disable())
                .authorizeHttpRequests((requests) -> requests
                        .requestMatchers("/vaultsystem/register", "/vaultsystem/invalidSession").permitAll()
                        .requestMatchers("/vaultsystem/*").authenticated()
                );
